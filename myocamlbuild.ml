...
let llvmconfig =
  let i = Unix.open_process_in "llvm-config --includedir" in
  let j = input_line i in
  let j = j ^ "/clang-c" in
  ignore (Unix.close_process_in i); j
    ...
    rule "generated-types exe"
      ...
      (fun _ _ ->
         ...
         Cmd (S [Sh cc; A"ffi_ml_types_stubgen.c";
                 A"-I"; P ctypes; A"-I"; P stdlib;A "-I"; A llvmconfig; A"-lclang";
